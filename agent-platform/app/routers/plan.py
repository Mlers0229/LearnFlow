...
async def generate_plan(goal: GoalRequest) -> PlanResponse:
    """兼容旧接口，仅返回基础计划结构。"""
    trace_id = str(uuid4())
    return orchestrator.run_study_plan(goal, trace_id=trace_id)
...
async def generate_plan_v2(goal: GoalRequest) -> PlanResponseV2:
    ...
    trace_id = str(uuid4())
    return orchestrator.run_study_plan_v2(goal, trace_id=trace_id)
...
async def replan_plan_v2(payload: PlanReplanRequest) -> PlanResponseV2:
    """对当前计划做 AI 重规划。"""
    trace_id = str(uuid4())
    return replan_agent.replan(payload, trace_id=trace_id)
...
async def refine_day(payload: DayRefineRequest) -> DayRefineResponse:
    """细化某一天的学习任务。"""
    trace_id = str(uuid4())
    return detail_agent.refine_day(payload, trace_id=trace_id)