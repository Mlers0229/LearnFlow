          ...
          <tr v-for="resource in resources" :key="resource.id" :class="{ risky: resource.invalidReportCount > 0, selected: selectedIds.includes(resource.id) }">
            ...
            <td>
              <div class="row-actions">
                ...
                <button v-if="normalizeStatus(resource.status) !== 'ACTIVE'" type="button" class="primary-action" @click="$emit('status', resource, 'ACTIVE')">上线</button>
                <button v-else type="button" @click="$emit('status', resource, 'INACTIVE')">下线</button>
              </div>
            </td>
          </tr>
        </tbody>
