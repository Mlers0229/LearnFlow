    ...
    <div v-if="enabled" class="lf-chat-context__resources">
      ...
      <div v-else-if="resources.length" class="lf-chat-context__resource-list">
        <button
          ...
          type="button"
          :disabled="!canViewResource(resource)"
          @click="openResource(resource)"
        >
          <span><strong>{{ resource.title || '学习资源' }}</strong><small>{{ resource.reason || resource.domain || resourceActionLabel(resource) }}</small></span>
...
import { ArrowUpRight, CalendarDays, ExternalLink, FileText, Library, ShieldCheck } from 'lucide-vue-next';
